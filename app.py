...
model = pickle.load(open('got_model.pkl', 'rb'))
...
def predict():
    ...
    # Retrieve symptoms values from the request form
    symptom_values = [int(request.form.get(symptom, 0)) for symptom in symptoms]

    # Create input query and convert it to numpy array
    input_query = np.array([symptom_values], dtype=int)
